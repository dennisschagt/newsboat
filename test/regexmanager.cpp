	...
	"[RegexManager]")
{
	...
	REQUIRE_NOTHROW(rxman.handle_action("highlight-article", R"("content =~ \"keyword\"" blue red)"));

	REQUIRE_NOTHROW(rxman.handle_action("highlight-article", R"("unread == \"yes\"" blue red bold underline)"));

	REQUIRE_NOTHROW(rxman.handle_action("highlight-article", R"("age > 3" blue red bold underline)"));
}
