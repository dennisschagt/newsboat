...
impl StflRichText {
    ...
    fn extract_style_tags(text: &str) -> (String, BTreeMap<usize, String>) {
        fn opt_first(a: Option<usize>, b: Option<usize>) -> Option<usize> {
            let Some(a) = a else {
                return b;
            };
            let Some(b) = b else {
        ...
        while pos < result.len() {
            ...
            let tag_start = opt_first(remainder.find('<'), remainder.find('>'));
            ...
            let tag_end = opt_first(remainder_in_tag.find('<'), remainder_in_tag.find('>'))
                .map(|i| i + tag_start + 1);
